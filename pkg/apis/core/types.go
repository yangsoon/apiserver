...
// copy from
// https://github.com/kubernetes/kubernetes/blob/0d90d1ffa5e87dfc4d3098da7f281351c7ff1972/pkg/apis/core/types.go
...
const (
	...
	// NamespaceAll is the default argument to specify on a context when you want to list or filter resources across all
	// namespaces
	NamespaceAll = ""
	...
	// TerminationMessagePathDefault means the default path to capture the application termination message running in a
	// container
...
type VolumeSource struct {
	...
	// PhotonPersistentDisk represents a Photon Controller persistent disk attached and mounted on kubelet's host
	// machine
...
const (
	// PersistentVolumeReclaimRecycle means the volume will be recycled back into the pool of unbound persistent volumes
	// on release from its claim.
	// The volume plugin must support Recycling.
	// DEPRECATED: The PersistentVolumeReclaimRecycle called Recycle is being deprecated. See announcement here:
	// https://groups.google.com/forum/#!topic/kubernetes-dev/uexugCza84I
	PersistentVolumeReclaimRecycle PersistentVolumeReclaimPolicy = "Recycle"
	// PersistentVolumeReclaimDelete means the volume will be deleted from Kubernetes on release from its claim.
	// The volume plugin must support Deletion.
	PersistentVolumeReclaimDelete PersistentVolumeReclaimPolicy = "Delete"
	// PersistentVolumeReclaimRetain means the volume will be left in its current phase (Released) for manual
...
const (
	// PersistentVolumeBlock means the volume will not be formatted with a filesystem and will remain a raw block
	// device.
...
type PersistentVolumeClaimSpec struct {
	...
	// (Alpha) Using the namespace field of dataSourceRef requires the CrossNamespaceVolumeDataSource feature gate to be
	// enabled.
	// +optional
...
type TypedObjectReference struct {
	...
	// Note that when a namespace is specified, a gateway.networking.k8s.io/ReferenceGrant object is required in the
	// referent namespace to allow that namespace's owner to accept the reference. See the ReferenceGrant documentation
	// for details.
...
const (
	...
	// PersistentVolumeClaimFileSystemResizePending - controller resize is finished and a file system resize is pending
	// on node
...
const (
	...
	// parameters such as nameservers and searcher paths should be defined via
	// DNSConfig.
...
type PodDNSConfig struct {
	...
	// +optional
	Nameservers []string
	// A list of DNS searcher domains for host-name lookup.
	// This will be appended to the base searcher paths generated from DNSPolicy.
	// Duplicated searcher paths will be removed.
	// +optional
	Searches []string
...
type ServiceAccount struct {
	...
	// Secrets is a list of the secrets in the same namespace that pods running using this ServiceAccount are allowed to
	// use. Pods are only limited to this list if this service account has a "kubernetes.io/enforce-mountable-secrets"
	// annotation set to "true".
	// This field should not be used to find auto-generated service account token secrets for use outside of pods.
	// Instead, tokens can be requested directly using the TokenRequest API, or service account token secrets can be
	// manually created.
...
type NodeSpec struct {
	...
	// Deprecated: Previously used to specify the source of the node's configuration for the DynamicKubeletConfig
...
type PodLogOptions struct {
	...
	// serving certificate of the backend it is connecting to.  This will make the HTTPS connection between the
	// apiserver
...
const (
	...
	// ServiceAccountRootCAKey is the key of the optional root certificate authority for SecretTypeServiceAccountToken
	// secrets
...
type TopologySpreadConstraint struct {
	...
	//
	// This is a beta field and requires the MinDomainsInPodTopologySpread feature gate to be enabled (enabled by
	// default).
	// +optional
	MinDomains *int32
	...
	//
	// This is a beta field and requires the MatchLabelKeysInPodTopologySpread feature gate to be enabled (enabled by
	// default).
	// +listType=atomic
	// +optional
